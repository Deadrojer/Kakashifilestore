...
async def get_dump_channel(self, user_id: int) -> Optional[Tuple[int, Optional[int]]]:
    """
    Returns:
        (channel_id, topic_id) 
        topic_id can be None
    """
    try:
        doc = await self.user_data.find_one(
            {"_id": int(user_id)},
            {"dump_channel": 1, "dump_topic_id": 1}
        )

        if doc and doc.get("dump_channel"):
            return (
                int(doc["dump_channel"]),
                int(doc["dump_topic_id"]) if doc.get("dump_topic_id") else None
            )
...
async def set_dump_channel(self, user_id: int, channel_id: int, topic_id: Optional[int] = None) -> bool:
    """
    Save user's dump channel and optional topic.
    """
    try:
        await self.user_data.update_one(
            {"_id": int(user_id)},
            {
                "$set": {
                    ...
                    "dump_topic_id": int(topic_id) if topic_id else None,
        ...
        logging.info(f"Dump set for {user_id} → {channel_id} | Topic → {topic_id}")
        return True
    ...
    """Remove user's saved dump channel and topic."""
    try:
